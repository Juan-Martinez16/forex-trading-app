...
io.on('connection', (socket) => {
  ...
  socket.on('assessOpportunities', async () => {
    try {
      const newOpportunities = await OpportunityService.assessAllPairs(marketData);
...
const updateMarketData = async () => {
  try {
    ...
    Object.keys(marketData).forEach(pair => {
      ...
      data.adx = Math.max(0, Math.min(100, data.adx + (Math.random() - 0.5) * 1));
...
cron.schedule('*/3 * * * * *', updateMarketData);
